...
### Как DPI определяет прокси-трафик

Современные системы DPI используют 4 уровня детекции:

1. **Сигнатурный анализ** — проверка первых 16-32 байт пакета против базы известных паттернов. Shadowsocks и OpenVPN выявляются на этом этапе. VLESS+TLS проходит — первые байты идентичны обычному HTTPS.

2. **TLS-fingerprinting (JA3/JA4)** — хэш параметров TLS ClientHello: список шифров, расширения, эллиптические кривые. Кастомные TLS-стеки (стандартная Go crypto/tls) имеют известные хэши и детектируются. **Решение**: REALITY использует fingerprint реального браузера (Chrome/Firefox), а Finalmask `fragment` разбивает ClientHello на фрагменты по 10-50 байт с задержкой 5-15 мс — DPI не может реассемблировать фрагменты на скорости линка, и JA3/JA4 хэш невозможно вычислить.

3. **Активное зондирование** — DPI отправляет разведочные пакеты на сервер. Легитимный сервис отвечает нормально, прокси — нет. **Решение**: REALITY при зондировании проксирует запрос на реальный сайт (microsoft.com), возвращая настоящую страницу.

4. **Поведенческий анализ (ML)** — нейросеть анализирует профиль трафика: размеры пакетов, тайминги, соотношение входящего/исходящего. Обычный VLESS over TCP генерирует постоянный двунаправленный поток с нетипичными размерами, отличными от реальных веб-сервисов. **Решение**: XHTTP разделяет восходящий и нисходящий трафик на отдельные HTTP-транзакции, каждая выглядит как обычная HTTP-пара. `xPaddingBytes` (100-1000 байт) нормализует размеры пакетов, `xmux` мультиплексирует потоки — профиль трафика становится неотличим от обычного веб-сёрфинга. Finalmask `sudoku` дополнительно трансформирует вид данных, исключая паттерны шифрованного трафика.

Подробнее: [DPI-детекция прокси-трафика в 2026](https://www.pvsm.ru/dpi/446489)

Предполагается что вы уже настроили серверную часть Xray-core v26.3.27+ с поддержкой XHTTP и Finalmask. Пример серверного конфига — [`example/server-config-example.json`](example/server-config-example.json).
