...
**Зачем нужна фрагментация TLS ClientHello?** Системы DPI (Deep Packet Inspection) анализируют первый TLS-пакет (ClientHello), который содержит поле SNI (Server Name Indication) и TLS fingerprint. Без фрагментации ClientHello отправляется единым пакетом размером до 16 384 байт (максимум TLS record). DPI перехватывает этот пакет целиком, извлекает SNI и сопоставляет fingerprint с известными паттернами прокси-протоколов (отличая их от реального браузерного трафика).

Finalmask `fragment` разбивает ClientHello на фрагменты по 10-50 байт с задержкой 5-15 мс между ними. DPI-оборудование, работающее в потоковом режиме, не может реассемблировать фрагменты на скорости линка — поле SNI оказывается разнесено по нескольким TCP-сегментам, а TLS fingerprint становится невозможно сопоставить с эталонным паттерном. При этом принимающая сторона (Xray-сервер) корректно собирает все фрагменты на уровне TCP и обрабатывает полный ClientHello.
